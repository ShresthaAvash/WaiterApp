  ...
  const notificationSound = new Sound('notification.mp3', Sound.MAIN_BUNDLE, (error) => {
      if (error) {
          ...
          Alert.alert("Sound Error", "Could not load the notification sound file. Please ensure 'notification.mp3' is in the correct folder and the app has been rebuilt.");
          return;
      }
  ...
  const TableScreen = ({ navigation }) => {
    ...
    const playNotificationSound = () => {
      ...
      if (notificationSound.isLoaded()) {
          notificationSound.play((success) => {
              ...
              notificationSound.setCurrentTime(0);
          });
      } else {
          console.error('Sound is not loaded, cannot play.');
      }
    };
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          if (readyTables.length > 0) {
            console.log('DETECTED READY TABLES:', readyTables.join(', '));
            Alert.alert(
              'Food Ready!',
    ...
    const handleClearTable = (table) => {
              ...
              [
                  { text: 'Cancel', style: 'cancel' },
                  {
                      ...
                      onPress: async () => {
                          try {
                              await clearTableStatus(table.id);
                              getTables(); // Refresh state from server
                          } catch (error) {
      ...
      if (table.status !== 'available') {
          let message = `This table has active kitchen orders (${table.status}) and cannot be cleared.`;
